...
(function (root) {
  ...
  const TITLE = 'House prices in England and Wales, as they were paid';
  ...
  function showError(error) {
    ...
    panel.className = 'loading';
    const title = document.createElement('h1');
    title.textContent = 'The house price data could not be loaded';
    const message = document.createElement('p');
    message.className = 'loading-message';
    ...
    host.append(panel);
    console.error('[house prices demo]', error);
  }
  ...
  /**
   * Start DuckDB WASM in the browser and register the Parquet file, returning
   * a grid source that queries it through the duckdbAdapter.
   *
   * The WASM bundle is imported from jsDelivr as an ES module (a classic
   * script may `import()` an ESM file without the page itself being one), and
   * the Parquet bytes are fetched and registered into DuckDB's virtual file
  ...
  async function buildDuckDBSource({ createPushdownSource, duckdbAdapter, update }) {
    ...
    update('Reading the Parquet file...', 0.6);
    const response = await fetch(root.HousePrices.PARQUET_URL);
    if (!response.ok) throw new Error(`The Parquet file answered ${response.status}.`);
    const buffer = new Uint8Array(await response.arrayBuffer());
    await db.registerFileBuffer('prices.parquet', buffer);
  ...
  async function start() {
    ...
    try {
      ...
      const { buildDashboard, readSnapshot, startPolling, POLL_MS, PARQUET_URL } = root.HousePrices;
      ...
      if (mode === 'snapshot') {
        ...
        try {
          ...
          /* DuckDB is out of our hands — a CDN that will not load the WASM, or
             a Parquet file that will not read — so a bad day for it should not
          ...
          console.warn('[house prices demo] the live DuckDB path failed, falling back to the saved copy:', liveError);
      ...
      /* The Parquet is read once into the browser; there is nothing to poll,
         and no poll is started after a fallback either. The feed's polling
         helpers exist for the snapshot tool, not for the DuckDB page. */
      built.poller = null;
      ...
      root.__housePrices = Object.assign(built, { meta, timings, duck, ready: true });
      console.log('[house prices demo] ready', timings);
    } catch (error) {
      root.__housePrices = { ready: false, error: String((error && error.message) || error) };
      showError(error);
    }
  }
